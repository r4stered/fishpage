...
owns. This is the one seam that turns a request into an actor, and where JWT verification would land
if the origin ever stopped being private.
"""
...
# Recorded as the Uploader off the Access edge — local ``just run`` and the test suite, where the
# header is absent. A missing identity must never fail a working upload, and an off-edge run has no
# real human to credit, so it is attributed to no one rather than rejected.
UNKNOWN_UPLOADER = "unknown"


def uploader_from_header(value: str | None) -> str:
    """The Uploader to credit, given the raw Access email header (or ``None`` when it is absent).

    A present email is taken at face value. An absent or blank header — every request off the Access
    edge — falls back to :data:`UNKNOWN_UPLOADER` so the upload still succeeds.
    """
